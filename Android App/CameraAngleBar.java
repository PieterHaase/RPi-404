...
public class CameraAngleBar extends PApplet {
    ...
    public void draw(){     // Wird kontinuierlich entsprechend der FrameRate aufgerufen
        ...
        strokeWeight(2);
        triangleXpos = width*cameraAngle/384;
    ...
    public void mousePressed(){
        //if (mainActivity.getAutoDrive() == false){
    ...
    public void mouseDragged(){
        if (mainActivity.getAutoDrive() == false && mainActivity.getCalibration() == false){
            if(touchDrag){
                int angle = mouseX * 384 / width;
                if(angle >= 0 && angle <= 384)
    ...
    public void setCameraAngle(float cameraAngle){ this.cameraAngle = Math.round(cameraAngle); }
